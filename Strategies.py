...
def ma_crossover_strategy(df, i: int):
	"""
...
def ichimoku_bullish(df, i: int):
	...
	if not df.__contains__('tenkansen')\
		or not df.__contains__('kijunsen') or not df.__contains__('senkou_a') or not df.__contains__('senkou_b'):
...
strategies_dict = dict(
	...
	ma_simple=ma_strategy,
	bollinger_simple=boll_strategy,
