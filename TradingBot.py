...
def backtest_strategies(
		symbols=None, interval='4h', plot=False, strategy_evaluators=None,
		options=None):
	if options is None:
		options = dict(
			starting_balance=100,
			nitial_profits=1.012,
	...
	for symbol in symbols:
		...
		for evaluator in strategy_evaluators:
			...
			if resulting_balance != trade_value:
				print(
					evaluator.strategy.__name__ + ": starting balance: " + str(trade_value)
...
def evaluate_strategies(symbols=None, strategy_evaluators=None, interval='1h', options=None):
	if strategy_evaluators is None:
		strategy_evaluators = []
	if symbols is None:
		symbols = []
...
def main():
	...
	if answer == 'e':
		evaluate_strategies(symbols=symbols, interval='1h', strategy_evaluators=strategy_evaluators)
	if answer == 'b':
		backtest_strategies(symbols=symbols, interval='1h', plot=False, strategy_evaluators=strategy_evaluators)
	if answer == 'q':
		print("\nBYE!\n")
